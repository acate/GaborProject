...
for Inc = 1:size(PosMat,1)
    ...
    OutStruct = MaxAmpfft2_noDC_chose_Amp(ImBlock.*GaussWin,4);
    ImBlockFilt = OutStruct.MatOut;
    AmpVec(Inc) = OutStruct.Amp; %%%% Write ImageMat blocks based on last paren. alone, ...
    % and collect list of all Amp Outs; then normalize so highest AmpOut is
    % 1, multiply ImageMat blocks by AmpBase*AmpOut and add Gray, in
    % another loop
