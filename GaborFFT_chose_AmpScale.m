...
for Inc = 1:size(PosMat,1)
    ...
    OutStruct = MaxAmpfft2_noDC_chose_Amp(ImBlock.*GaussWin,3);
    ImBlockFilt = OutStruct.MatOut;
    AmpOut = OutStruct.Amp; %%%% Write ImageMat blocks based on last paren. alone, ...
    ...
    Block = Gray+(AmpBase*AmpOut)*(ImBlockFilt.*GaussWin);
    ImageMat(...
        PosMat(Inc,1) - (BlockSize-1)/2:PosMat(Inc,1) + (BlockSize-1)/2, ...
        PosMat(Inc,2) - (BlockSize-1)/2:PosMat(Inc,2) + (BlockSize-1)/2 ...
        ) = ...
