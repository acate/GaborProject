...
for Inc = 1:floor(MaxSize/3);

ImageMatSize = size(InputImMat);
ImageMat = ones(ImageMatSize);

Band = Inc; %integer from 1 to half of GaussWin radius (if don't want
          %greater than Nyquist freq).  

BlockSize = MaxSize; %odd number because meshgrid makes odd-number sized grids
GapSize = 0;
...
imwrite(uint8(ImageMat),...
        [FileNameBase '_block' num2str(BlockSize) '_gap' num2str(GapSize) ...
         'band_' num2str(Band) '_MaxSize' num2str(MaxSize) '.png'],'png')
